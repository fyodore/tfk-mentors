...
function AttendanceStatusBadge({ showUp }) {
  const className =
    showUp === 'attended'
...
export default function AttendancePage() {
  ...
  const practiceIdFromRoute = routeId ? Number.parseInt(String(routeId), 10) : null
  ...
  function applyPracticePayload(payload) {
    setPractice(payload)
  ...
  async function loadArchived(seasonId) {
  ...
  useEffect(() => {
    ...
    Promise.resolve().then(async () => {
      try {
        const rows = await fetchArchivedPracticeAttendance({ season: seasonFilter })
        if (!cancelled) setArchived(Array.isArray(rows) ? rows : [])
      } catch (e) {
        if (!cancelled) {
          setError(e instanceof Error ? e.message : String(e))
  ...
  function setMentorShowUp(mentorId, showUp) {
    setShowUpByMentor((prev) => ({ ...prev, [mentorId]: showUp }))
  }

  function openArchivedPractice(practiceId) {
          ...
          <>
            ...
            <section className="attendance-section">
              ...
              <div className="attendance-save-row">
                ...
                {saveMessage ? (
                  <span className="muted attendance-save-message">{saveMessage}</span>
                ) : null}
              </div>
            </section>
          </>
                ...
                <tbody>
                  {archived.map((row) => {
                    ...
                    return (
                      <tr
                        key={row.practice_id}
                        className={isSelected ? 'attendance-archive-selected' : ''}
                      >
                        <td>
                          <button
                            type="button"
                            className="btn-text attendance-archive-link"
                            onClick={() => openArchivedPractice(row.practice_id)}
                          >
                            ...
                            {sortByPaceThenName(row.assigned_mentors ?? []).map(
                              (mentor) => (
                                <li key={mentor.mentor_id}>
                                  {mentorName(mentor)}
                                  {' — '}
                                  <AttendanceStatusBadge showUp={mentor.show_up} />
                                </li>
                              )
                            )}
                          </ul>
